...
1. **Marketplace install** — `/plugin marketplace add astriaai/astria-claude-skills`
   installs the `astria` plugin from `plugins/astria/`. See `README.md`.
...
```
.claude-plugin/marketplace.json     marketplace catalog → the `astria` plugin
plugins/astria/
  .claude-plugin/plugin.json        plugin manifest
  bin/astria                        the astria CLI — VENDORED, see below
  hooks/hooks.json                  SessionStart auth check
  skills/<name>/SKILL.md            public skills
scripts/sync-cli.sh                 re-vendor the CLI from astriaai/cli
README.md                           marketplace install + usage
```

## The `astria` CLI

`plugins/astria/bin/astria` is the single entry point for the Astria API. It
...
`plugins/astria/bin/astria` is a *vendored copy* so marketplace installs and the
...
- Document any new API surface as `astria` CLI verbs in the `astria-api` skill;
  add the verb to `plugins/astria/bin/astria`.

## Testing

```bash
plugins/astria/bin/astria --help
python3 -m py_compile plugins/astria/bin/astria
plugins/astria/bin/astria login --api-key <key> && plugins/astria/bin/astria whoami
```
